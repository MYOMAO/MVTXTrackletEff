...
{
	std::cout << "Now processing - ZZ MvtxHitEff - use track map -> Find nskip -> Changed ZZ -> Vertex" << std::endl; 	
		...
		for (TrackSeed::ClusterKeyIter cluskey = cluskeybegin; cluskey != cluskeyend; ++cluskey){ //looping through clusters
			...
			if (TrkrDefs::getTrkrId(*cluskey) == TrkrDefs::mvtxId || TrkrDefs::getTrkrId(*cluskey) == TrkrDefs::inttId){
				...
				if (TrkrDefs::getTrkrId(*cluskey) == TrkrDefs::mvtxId){
					...
					if(size > mvtxclusmax) goodclussize = false;
				}
				
				if (TrkrDefs::getTrkrId(*cluskey) == TrkrDefs::inttId){				
					INTTClusSize->Fill(size);	
					if(size > inttclusmax) goodclussize = false;
