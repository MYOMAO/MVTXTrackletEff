...
PHActsSiliconSeeding::PHActsSiliconSeeding(const std::string& name)
	: SubsysReco(name)
{
	std::cout << "OK - ZZ Modi PHActsSiliconSeeding - ok new 2 -> New" << std::endl;
}
...
{
	std::cout << "OK seed mody ZZ " << std::endl;
	int numSeeds = 0;
	int numGoodSeeds = 0;
	m_seedid = -1;
		...
		for (auto& seed : seeds)
		{
			...
			for (auto& spacePoint : seed.sp())
			{
				...
				if(TrkrDefs::getTrkrId(cluskey) == TrkrDefs::mvtxId){
					...
					std::cout << "MVTX Seed: " <<  "  Layer - " << layer << std::endl;


					if(layer < -1){
						std::cout << "Encounter Layer 0 - Skip Manually -> Good" << std::endl;
			...
			//	  std::cout << "Pass 4.03" << std::endl;

			trackSeed->set_phi(phi);  // make phi persistent
									  /// Project to INTT and find matches
									  //	  std::cout << "Pass 4.05" << std::endl;

			int mvtxsize = globalPositions.size();
			//	  std::cout << "Pass 4.08" << std::endl;
			//	  std::cout << "no use additional clus from my side = " << mvtxsize << std::endl;
			...
			std::cout << "mvtxsize = " << mvtxsize << "   additionalClusters.size() = " << additionalClusters.size() << std::endl;
...
std::vector<TrkrDefs::cluskey> PHActsSiliconSeeding::findMatches(
		...
		TrackSeed& seed)
{
	std::cout << "Inside: find matches: OK - Good - Pass 0" << std::endl;	
	...
	//  std::cout << "Pass 4: PHActsSiliconSeeding" << std::endl;	
	...
	for (auto& det : {TrkrDefs::TrkrId::mvtxId, TrkrDefs::TrkrId::inttId})
	{
		...
		while (layer < nlayers)
		{
			...
			for (const auto& hitsetkey : m_clusterMap->getHitSetKeys(det, layer))
			{
				...
				//	    std::cout << "Pass 4: PHActsSiliconSeeding - Pass 5" << std::endl;	
				...
				for (auto clusIter = range.first; clusIter != range.second; ++clusIter)
				{
					...
					m_projlz = local.y();
					/// Diagnostic
					//	      std::cout << "Pass 4: PHActsSiliconSeeding - Pass 7" << std::endl;	
					...
					//	      std::cout << "Pass 4: PHActsSiliconSeeding - Pass 8" << std::endl;	
					...
					// 	      std::cout << "Pass 4: PHActsSiliconSeeding - Pass 9" << std::endl;	
...
std::vector<const SpacePoint*> PHActsSiliconSeeding::getSiliconSpacePoints(Acts::Extent& rRangeSPExtent)
{
	...
	for (const auto& det : dets)
	{
		for (const auto& hitsetkey : m_clusterMap->getHitSetKeys(det))
		{
			...
			{
				const auto cluskey = clusIter->first;
				int LayerZZ = TrkrDefs::getLayer(cluskey);
				if(LayerZZ == 1) continue;
				totNumSiliconHits++;
				if (_iteration_map != nullptr && m_nIteration > 0)
				{
					if (_iteration_map->getIteration(cluskey) < m_nIteration)
