...
void SelectBest(int RunID){

	gStyle->SetOptStat(0);
	TCanvas * c = new TCanvas("c","c",600,600);
	c->cd();

	TString infile = Form("NewFiles/Run%d.root",RunID);
	...
	TString outfile =  Form("CutFiles/Run%d.root",RunID);
	...
	TFile * SaveHis = new TFile(Form("HisSave/SaveHis%d.root",RunID),"RECREATE");
	SaveHis->cd();
	...
	for(int q = 0; q < NTree; q++){
		...
		TH1D * ReuseSiSeed = new TH1D(Form("ReuseSiSeed_%d",q),"",100,0,200);
		...
		TH1D * DeltaX = new TH1D(Form("DeltaX_%d",q),"",100,-10,10);
		...
		DeltaX->SetMarkerColor(1);
		DeltaX->SetLineColor(1);

		TH1D * DeltaXAfter = (TH1D *) DeltaX->Clone(Form("DeltaXAfter_%d",q));
		DeltaXAfter->SetMarkerColor(2);
		DeltaXAfter->SetLineColor(2);

		TH1D * DeltaY = new TH1D(Form("DeltaY_%d",q),"",100,-10,10);
		...
		DeltaY->SetMarkerColor(1);
		DeltaY->SetLineColor(1);

		TH1D * DeltaYAfter = (TH1D *) DeltaY->Clone(Form("DeltaYAfter_%d",q));
		DeltaYAfter->SetMarkerColor(2);
		DeltaYAfter->SetLineColor(2);
		...
		DeltaZ->SetMarkerColor(1);
		DeltaZ->SetLineColor(1);

		TH1D * DeltaZAfter = (TH1D *) DeltaZ->Clone(Form("DeltaZAfter_%d",q));
		DeltaZAfter->SetMarkerColor(2);
		DeltaZAfter->SetLineColor(2);
		...
		NTracks->SetMarkerColor(1);
		NTracks->SetLineColor(1);


		TH1D * NTracksAfter = (TH1D *) NTracks->Clone(Form("NTracksAfter_%d",q));
		NTracksAfter->SetMarkerColor(2);
		NTracksAfter->SetLineColor(2);
		...
		for(int i = 0; i < NTracklets; i++){
			...
			if (BestFit.find(siseedpt) == BestFit.end() || 
					std::abs(BestFit[siseedpt].second - 1) > quality) {
				BestFit[siseedpt] = {i, quality};
			}
...
*/
		...
		c->SaveAs(Form("SelPlot/%d/ReuseSiSeed_%d.png",RunID,q));
		...
		c->SaveAs(Form("SelPlot/%d/Match//DeltaX_%d.png",RunID,q));
		...
		c->SaveAs(Form("SelPlot/%d/Match/DeltaY_%d.png",RunID,q));
		...
		c->SaveAs(Form("SelPlot/%d/Match/DeltaZ_%d.png",RunID,q));
		...
		NTracks->Draw("epSAME");
		leg2->Draw("epSAME");
		c->SaveAs(Form("SelPlot/%d/Match/NTrack_%d.png",RunID,q));
