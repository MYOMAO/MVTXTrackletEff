...
void SelectBestTPC(int RunID){

	gStyle->SetOptStat(0);
	TCanvas * c = new TCanvas("c","c",600,600);
	c->cd();

	TString infile = Form("CutFiles/Run%d.root",RunID);
	...
	TString outfile = Form("CutFilesFinal/Run%d.root",RunID);
	...
	TFile * SaveHis = new TFile(Form("HisSave/SaveHisTPC%d.root",RunID),"RECREATE");
	SaveHis->cd();
...
*/
		...
		c->SaveAs(Form("SelPlotTPC/%d/ReuseSiSeed_%d.png",RunID,q));
		...
		c->SaveAs(Form("SelPlotTPC/%d/Match/DeltaX_%d.png",RunID,q));
		...
		c->SaveAs(Form("SelPlotTPC/%d/Match/DeltaY_%d.png",RunID,q));
		...
		c->SaveAs(Form("SelPlotTPC/%d/Match/DeltaZ_%d.png",RunID,q));
		...
		NTracks->Draw("epSAME");
		leg2->Draw("epSAME");
		c->SaveAs(Form("SelPlotTPC/%d/Match/NTrack_%d.png",RunID,q));
