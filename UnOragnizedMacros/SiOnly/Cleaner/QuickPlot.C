...
//	TFile * fin = new TFile("Files/SiTrackFit.root");
    ...
    TFile * fin = new TFile("Clean3.root");
	...
	MissingChip->SetBranchAddress("TotalClusSize",&TotalClusSize);
	MissingChip->SetBranchAddress("ClusPosX",&ClusPosX);
	MissingChip->SetBranchAddress("ClusPosY",&ClusPosY);
	MissingChip->SetBranchAddress("ClusPosZ",&ClusPosZ);
	...
	//	CosTheta = (tpcseedpxall * siseedpxall  + tpcseedpyall  * siseedpyall  + tpcseedpzall  * siseedpzall )/(sqrt(tpcseedpxall  * tpcseedpxall  + tpcseedpyall  *tpcseedpyall  + tpcseedpzall  * tpcseedpzall ) * sqrt(siseedpxall  * siseedpxall  + siseedpyall  * siseedpyall  + siseedpzall  * siseedpzall ));
		...
		PassClus = true;
		//Include Seed Studies//

		TotalClusSize = ClusPosX->size();
//		cout << "TotalClusSize = " << TotalClusSize << endl;
		...
		if(quality < 100) Layer[1]->Fill(layerindex);
		if(trackpt > 0.5 && quality < 5) Layer[2]->Fill(layerindex);
		if(PassClus == true) Layer[3]->Fill(layerindex);
		...
		if(ndf != -1 && CosTheta > 0.9  && trackpt > 0.5 )  Layer[4]->Fill(layerindex);
*/
	...
	TotalTrack->SetBranchAddress("six",&sixall);
	TotalTrack->SetBranchAddress("siy",&siyall);
	...
	for(int i = 0; i < NTrackletsFired; i++){
		...
		for(int j = 0; j < TotalClusSize; j++){
			...
			ROOT::Math::XYZVector DP(siseedpxall,siseedpyall,siseedpzall);
		...
		if(qualityall < 100) NTrackletPassed[1]++;
		if(trackptall > 0.5 && qualityall < 5) NTrackletPassed[2]++;
		if(PassClus == true) NTrackletPassed[3]++;

		if(PassClus == true) NewQual->Fill(qualityall);
	...
	/*	
		...
		if(ndf != -1 && CosTheta > 0.9  && trackptall > 0.5 )  NTrackletPassed[4]++;
	*/
