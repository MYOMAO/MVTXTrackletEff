...
void Step2(int Run){
	...
	TString infile = Form("Outfile/Clean1_%d.root",Run);
	TFile * fin = new TFile(infile.Data());
	fin->cd();


	TString infile2 = Form("Outfile/Clean1_%d.root",Run);
	TFile * fin2 = new TFile(infile2.Data());
	fin2->cd();


	TFile * fout = new TFile(Form("Outfile/Clean2_%d.root",Run),"RECREATE");
	fout->cd();
