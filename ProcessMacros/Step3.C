...
void Step3(int Run){
	...
	TString infile = Form("Outfile/Clean2_%d.root",Run);
	TFile * fin = new TFile(infile.Data());
	fin->cd();


	TString infile2 = Form("Outfile/Clean2_%d.root",Run);
	...
	TString outfile = Form("Outfile/Clean3_%d.root",Run);
	...
	for(int i = 0; i < NMissing; i++){
		...
		auto it3 = std::find(AllTrackletID[position].begin(), AllTrackletID[position].end(), TrackletID2);


		if(it3 != AllTrackletID[position].end()){
